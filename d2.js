...
const passwords = input.split("\n").map(l => ({
  lim: l.substring(0, l.indexOf(" ")).split("-").map(n => parseInt(n)),
  char: l.substring(l.indexOf(" ") + 1, l.indexOf(":")),
  password: l.substring(l.lastIndexOf(" ") + 1)
}));

const validPasswords1 = passwords.filter(({lim: [min, max], char, password}) => {
  const charCount = password.split("").filter(c => c === char).length;
  return charCount >= min && charCount <= max;
});

const validPasswords2 = passwords.filter(({lim: [pos1, pos2], char, password}) => {
